...
<!-- 在线安装需要您分别编译安装 MatPL 和 lammps 接口。完整的在线安装依赖`intel 编译器`、`CUDA-11.8工具包`、`gcc编译器`以及 `Python 环境`。
- 对于 `intel/2020`编译套件，用于编译支持MatPL NN 和 Linear 模型，使用了 `ifort` 和 `icc` 编译器(`19.1.3`)、`mpi(2019)`、`mkl库(2020)`，如果单独加载，请确保版本不低于它们。如果不使用 NN 和 Linear 模型（只使用 DP 或 NEP 模型），可以不加载该编译环境。

- MatPL 训练采用了 GPU 设备和 Pytorch，因此要求 CUDA 编译环境(>= 11.8) 和 带 pytorch 的 Python 环境(要求 python >= 3.11，torch >= 2.2.0+cu118)。

- `MatPL 和 lammps-MatPL 提供了纯 CPU 版本`。对于纯 CPU 版本，不再需要 CUDA 编译环境。
...
#### 编译安装-检查编译环境(CPU 版本)
安装 GPU 版本请跳过该步。对于 CPU 版本，不需要 CUDA 支持，检测脚本为 check_env_cpu.sh，位于 `'/src/check/check_env_cpu.sh'`。命令执行后会列出需要的编译器版本以及当前检测到的版本，如下是一个正确的环境配置检查后的结果：
```
ifort version is no less than 19.1, current version is 19.1.
MKL library is installed.
GCC version is exactly 8, current version is 8.
...
#### 下载 lammps-MatPL (libtorch 版本) 源码
...
##### 为了使用 NEP模型的 GPU 版本，需要您先将 NEP 的 c++ cuda 代码编译为共享库文件
...
#### 复制Lammps-MatPL (fortran)源码到Lammps源码下
