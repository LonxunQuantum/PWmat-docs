...
## wuxing
date=$(date +"%Y-%m-%d")
output="MatPL-2026.3_userdoc3.pdf"
...
awk '{
    ...
    if (!in_front) print
}' "models/README.md" | sed 's|\./pictures/|models/pictures/|g' >> temp.md
echo "" >> temp.md
...
for f in "${nep_files[@]}"; do
    ...
    awk '{
        ...
        if (!in_front) print
    }' "models/nep/$f" | sed 's|\./pictures/|models/nep/pictures/|g' >> temp.md
    echo "" >> temp.md
done
...
for f in "${dp_files[@]}"; do
    ...
    awk '{
        ...
        if (!in_front) print
    }' "models/dp/$f" | sed 's|\./pictures/|models/dp/pictures/|g' >> temp.md
    echo "" >> temp.md
done
...
for f in "${nn_files[@]}"; do
    ...
    awk '{
        ...
        if (!in_front) print
    }' "models/nn/$f" | sed 's|\./pictures/|models/nn/pictures/|g' >> temp.md
    echo "" >> temp.md
done
...
for f in "${pwact_files[@]}"; do
    if [ -f "pwact/$f" ]; then
        ...
        awk '{
            ...
            if (!in_front) print
        }' "pwact/$f" | sed 's|\./pictures/|pwact/pictures/|g' >> temp.md
        echo "" >> temp.md
    fi
done
echo "\newpage" >> temp.md
...
for f in "${pwdata_files[@]}"; do
    if [ -f "pwdata/$f" ]; then
        ...
        awk '{
            ...
            if (!in_front) print
        }' "pwdata/$f" | sed 's|\./pictures/|pwdata/pictures/|g' >> temp.md
        echo "" >> temp.md
    fi
done
echo "\newpage" >> temp.md
...
for f in "${examples_files[@]}"; do
    if [ -f "examples/$f" ]; then
        ...
        awk '{
            ...
            if (!in_front) print
        }' "examples/$f" | sed 's|\./pictures/|examples/pictures/|g' >> temp.md
        echo "" >> temp.md
    fi
done
echo "\newpage" >> temp.md
