...
- 提取各种轨迹文件 `MOVEMENT(PWmat)`、`OUTCAR(VASP)`、`lammps dump file(Lammps)`、`cp2k md file(CP2K)` 或常用训练数据` pwmlff/npy`、`extxyz`、`deepmd/npy`、`deepmd/raw`、[`meta OMAT24 开源数据集`](https://huggingface.co/datasets/fairchem/OMAT24) 之后转换为 `pwmlff/npy` 或者 `extxyz` 格式文件。
...
- `pwmlff/npy` 用于保存数据集的标签。它可以用于训练机器学习模型。
...
### pwdata 接口代码案例-把MPtraj文件转换为lmdb格式
