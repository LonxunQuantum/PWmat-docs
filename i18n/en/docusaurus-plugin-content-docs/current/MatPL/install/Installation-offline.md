...
是否安装成功检查：
编译完成后，MatPL_cpu-2026.3 目录下生成如下目录结构：
```txt
MatPL-2026.3 (或MatPL_cpu-2026.3)
    ├── lammps-2026.3/
    ├── lammps-fortran/
    ├── matpl-2026.3/  (或 matpl_cpu-2026.3)
    ├── MatPL-2026.3/  (或 MatPL_cpu-2026.3)
    ├── matpl-env.sh
...
MatPL 在使用时需要先加载编译时使用的 CUDA （对于GPU版本），如果训练 NN 或 Linear 还需要加载 mlk库。之后执行如下命令 即可完成 python 环境、 MatPL-2026.3 和 lammps 的加载。
```bash
